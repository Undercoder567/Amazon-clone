...
function Signup() {
  ...
  const CreateUser = async () => {
    try {
      ...
      await checkAndAddUser(user.uid, email);
      navigate("/home");
    } catch (error) {
      swal({
        title: "Error!",
  ...
  const GoogleAuth = async () => {
    try {
      ...
      await checkAndAddUser(user.uid, user.email);
      
      // Navigate to home after successful sign-in and user check
      navigate("/home");
    } catch (error) {
      swal({
        title: "Error!",
  ...
  const checkAndAddUser = async (uid,email) => {
    const userDocRef = doc(db, "Users", uid);
    const userDoc = await getDoc(userDocRef);

    if (!userDoc.exists()) {
      // Add user document with uid as document ID
