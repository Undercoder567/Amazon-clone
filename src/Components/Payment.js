  ...
  const AddUserData = async () => {
    try {
      ...
      if (user) {
        ...
        const orderId = String(OrderID); // Convert OrderID to a string
  
        console.log("User ID:", userId);
        console.log("Order ID:", orderId);
  
        // Reference to the Orders subcollection within the user's document
        const ordersDocRef = doc(db, "Users", userId, "Orders", orderId); // Use OrderID as document ID
  
        // Set the document with the order details
